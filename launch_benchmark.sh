...
function main {
    ...
    do
        # cache weight
        python examples/imagenet_eval.py -e --performance \
            --pretrained None --dummy -j 1 -w 1 -i 2 \
            -a $model_name -b 1 --precision ${precision} \
            --channels_last ${channels_last} \
            --config_file ${workload_dir}/conf.yaml ${addtion_options}
        #
...
function generate_core_launcher {
    ...
    do
        ...
        printf "python -m oob-common.launch --enable_jemalloc \
            ...
            examples/imagenet_eval.py -e --performance --pretrained None --dummy -j 1 \
                --config_file ${workload_dir}/conf.yaml \
                -w ${num_warmup} -i ${num_iter} \
                -a ${model_name} \
                -b ${batch_size} \
